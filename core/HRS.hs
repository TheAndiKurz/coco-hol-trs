...
import Data.List (groupBy, nub, partition, sort, sortOn)
...
checkFreeVars vars = do
  ...
  where
    ...
    check (v1@(Var _ typ) : v2 : vs)
      | varId v1 == varId v2 && varType v1 /= varType v2 =
          Left $
            "free variable '"
              ++ show v1
              ++ "' occures twice and has different types: "
              ++ show (varType v1)
              ++ " and "
              ++ show (varType v2)
...
getTermType system bound_vars term@(Term id args) = case findVar (bound_vars ++ functions system) id of
  Just (Var _ (Type fargs _))
    | length fargs /= length args ->
        Left $
          "function '"
            ++ show id
            ++ "' expects "
...
duplicateVarsCheck desc vars = case firstDuplicate vars of
  Just (Var id _) -> Left $ "every " ++ desc ++ " has to have a unique name. Duplicated variable name: " ++ show id
  Nothing -> return ()

sameTypes :: [Type] -> [Type] -> Bool
sameTypes ts1 ts2
  | length ts1 /= length ts2 = False
  | otherwise = all (uncurry (==)) $ zip ts1 ts2
...
etaReduce (TermLambda lambda_vars (Term f args)) =
  let reduceTail :: [Var] -> [Term] -> ([Var], [Term])
...
hasFree ids (Term term_id term_args) = notElem term_id ids || any (hasFree ids) term_args
    ...
    then return Nothing
      ...
      case groupTypeClasses type_classes1 type_classes2 of
        ...
        Just function_groups -> do
          let formula =
                Smt.conj
                  [ sortSmt (sorts system1) (sorts system2),
...
typeSameSkeleton (Type args1 _) (Type args2 _)
  ...
  | otherwise = and $ zipWith typeSameSkeleton args1 args2
...
sortSmt sorts1 sorts2 = Smt.conj [distinct_sorts1, distinct_sorts2, one_match]
  where
    ...
    one_match =
      Smt.conj
        [ Smt.disj [sortEq sort1 sort2 | sort2 <- sorts2]
          | sort1 <- sorts1
        ]
...
termsEq (Term tid1 args1) (Term tid2 args2)
  ...
  where
    argsEq = zipWith termsEq args1 args2
